(function (AddressData, RiskData) {
    ...
    // add event listner on key up with
    // debounce
    if (addressField) {
        addressField.addEventListener('keyup', debounce(inputChange, 300))
    }

    // add event listner on click with
    if (addressList) {
    ...
    function renderResult(data) {
        if (addressList) {
            for (address of data) {
                ...
                li.setAttribute('data-geo', JSON.stringify(address.geometry.coordinates));
